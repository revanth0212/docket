...
description: Build plugins and adapters for Cortex without forking the project.
---

# Developer Overview

Cortex is built around **adapter contracts**. You can extend it by
...
```
Your Adapter  ──implements──▶  Interface Contract  ◀──called by──  Cortex Core
```
...
Replace how Cortex talks to LLMs, databases, blob stores, or queues.

Example: adding a new vector database.

### 2. Connectors (data sources)

Ingest data from external systems into Cortex.
...
use Cortex's API (or direct adapter injection) to push data in.
...
```
@cortex/{category}-{provider}
```

Examples:
- `@cortex/llm-groq`
- `@cortex/store-qdrant`
- `@cortex/connector-github`
