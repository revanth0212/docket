...
Cortex will expose **two Model Context Protocol (MCP) servers** — one for the data plane and one for the control plane. This mirrors our HTTP architecture: users interact with data, operators interact with configuration.

> **Status**: Planned for post-v0.2.0. Track progress on the [roadmap](/docs/developers).

## Why MCP?

MCP lets Claude Desktop, Cursor, Windsurf, and any MCP-compatible client interact with Cortex as a native tool. Instead of writing HTTP calls, users can:

- Ask Claude *"What did I photograph last week?"* and Cortex answers via the MCP data plane.
...
│  · cortex_query     │       │  · cortex_health    │
│  · cortex_ingest    │       │  · cortex_config    │
│  · cortex_get       │       │  · cortex_plugin_add│
│  · cortex_create    │       │  · cortex_plugin_rm │
│  · cortex_update    │       │  · cortex_rbac_list │
│  · cortex_delete    │       │  · cortex_rbac_set  │
│  · cortex_relate    │       │                     │
│                     │       │  Resources exposed: │
...
|------|---------|---------|
| `cortex_query` | Natural language search | `POST /query` |
| `cortex_ingest` | Ingest text or file path | `POST /ingest` |
| `cortex_get` | Retrieve a memory by ID | `GET /memories/:id` |
| `cortex_create` | Create a memory directly | `POST /memories` |
| `cortex_update` | Update a memory | `PATCH /memories/:id` |
| `cortex_delete` | Delete a memory | `DELETE /memories/:id` |
| `cortex_relate` | Create a relation between memories | `POST /memories/:id/relations` |
...
For **operators and developers** who manage the Cortex instance.

### Tools

| Tool | Purpose | Maps to |
|------|---------|---------|
| `cortex_health` | Aggregated adapter health | `GET /admin/health` |
| `cortex_config` | View or reload runtime config | `GET/POST /admin/config` |
| `cortex_plugin_add` | Onboard a new adapter | `POST /admin/plugins` |
| `cortex_plugin_rm` | Deregister an adapter | `DELETE /admin/plugins/:name` |
| `cortex_rbac_list` | List access policies | `GET /admin/rbac/policies` |
| `cortex_rbac_set` | Create or update a policy | `POST /admin/rbac/policies` |
...
Claude → calls cortex_plugin_add({
  packageName: "cortex-llm-groq",
  config: { apiKey: "gsk_...", model: "llama3-70b-8192" }
})

Cortex → validates package, initializes adapter, returns metadata
...
```json
{
  "mcpServers": {
    "cortex-data": {
      "command": "npx",
      "args": ["-y", "cortex-mcp-data", "--baseUrl", "http://localhost:3000"]
    },
    "cortex-control": {
      "command": "npx",
      "args": ["-y", "cortex-mcp-control", "--baseUrl", "http://localhost:3001"]
    }
  }
}
```
...
```bash
export CORTEX_DATA_URL=http://localhost:3000
export CORTEX_CONTROL_URL=http://localhost:3001
npx cortex-mcp-data
```
