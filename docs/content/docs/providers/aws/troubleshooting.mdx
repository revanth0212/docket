...
```bash
aws dynamodb describe-table --table-name cortex --region us-east-1
```
...
```bash
aws sqs get-queue-url --queue-name cortex-jobs
```
...
1. Deploy Cortex closer to users (multi-region)
