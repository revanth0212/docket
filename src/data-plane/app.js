...
const { buildPrincipalExtractor } = require('./middleware/principal');
...
async function registerDataPlaneRoutes(app, options = {}) {
  ...
  const rbacConfig = config.docket?.memory?.rbac;
  const rbacEnabled = rbacConfig?.enabled === true;
  ...
  // Principal extraction + per-request services when RBAC is enabled
  if (rbacEnabled) {
    const extractPrincipal = buildPrincipalExtractor(rbacConfig);
    app.addHook('preHandler', async (request, reply) => {
      await extractPrincipal(request, reply);
      if (reply.sent) return;

      request.services = createCoreServices(adapters, config, { principal: request.principal });
    });
  }
  ...
  await app.register(registerIngestRoute, {
    ingestionService: services.ingestion,
    getIngestionService: (request) => request.services?.ingestion || services.ingestion
  });

  // Query
  await app.register(registerQueryRoute, {
    queryService: services.query,
    getQueryService: (request) => request.services?.query || services.query
  });

  // Memory CRUD + relations
  await app.register(registerMemoryRoutes, {
    memoryService: services.memory,
    getMemoryService: (request) => request.services?.memory || services.memory
  });
}
